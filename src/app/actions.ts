...
import type { PlaywrightPulseReport } from '@/types/playwright'; // Removed HistoricalTrend, using PlaywrightPulseReport
...
export async function getFailurePatternAnalysis(): Promise<{ success: boolean; analysis?: string; error?: string }> {
  try {
    ...
    const historicalDataArray: PlaywrightPulseReport[] = []; // Corrected type to PlaywrightPulseReport[]
    for (const fileName of trendFileNames) {
      const filePath = path.join(historyDir, fileName);
      const fileContent = await fs.readFile(filePath, 'utf-8');
      historicalDataArray.push(JSON.parse(fileContent) as PlaywrightPulseReport); // Parse as PlaywrightPulseReport
    }
