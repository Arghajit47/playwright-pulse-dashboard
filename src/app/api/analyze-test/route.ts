  ...
  try {
    ...
    if (!testName || !failureLogsAndErrors) {
      return NextResponse.json(
        {
          message: "Missing required fields: testName and failureLogsAndErrors",
        },
        { status: 400 }
      );
    }
    ...
    const externalApiResponse = await fetch(
      "https://ai-test-analyser.netlify.app/api/analyze",
      {
        method: "POST",
        headers: {
    ...
    if (!externalApiResponse.ok) {
      ...
      console.error(
        `External API error: ${externalApiResponse.status}`,
        errorBody
      );
      return NextResponse.json(
        { message: `Error from external API: ${errorBody}` },
