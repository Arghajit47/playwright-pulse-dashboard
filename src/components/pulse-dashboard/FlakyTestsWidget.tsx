    ...
    return (
      <Accordion type="multiple" className="w-full space-y-3 mt-4">
        {tests.map((test, index) => (
          <AccordionItem value={`flaky-${test.id}-${index}`} key={test.id + index} className="border rounded-2xl bg-card hover:bg-muted/20 transition-colors">
            ...
            <AccordionContent className="p-4 pt-0">
              ...
              <div className="max-h-60 overflow-y-auto space-y-1.5 pr-2">
                {test.occurrences.map((occ: FlakyTestOccurrence, occIndex: number) => (
                  <div key={occIndex} className="flex justify-between items-center text-xs p-1.5 bg-muted/30 rounded-xl">
