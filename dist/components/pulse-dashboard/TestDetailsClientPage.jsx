...
import { ArrowLeft, CheckCircle2, XCircle, AlertCircle, Clock, ImageIcon, FileText, LineChart, Info, Download, Film, Archive, Terminal, FileJson, FileSpreadsheet, FileCode, File as FileIcon, Copy, Check, } from "lucide-react";
...
const CodeBlockWithCopy = ({ rawContent, className, preClassName, }) => {
    ...
    return (<div className={cn("relative", className)}>
      <pre className={cn("text-sm p-3 pr-12 whitespace-pre-wrap break-all font-code", preClassName)}>
        <span dangerouslySetInnerHTML={{ __html: ansiToHtml(rawContent || "") }}/>
      </pre>
...
export function TestDetailsClientPage({ testId }) {
    ...
    const screenshotDisplayItems = useMemo(() => {
    ...
    const videoDisplayItems = useMemo(() => {
        if (!test || !test.videoPath || !Array.isArray(test.videoPath))
            return [];
    ...
    const traceDisplayItem = useMemo(() => {
    ...
    const textCsvAttachments = useMemo(() => allOtherAttachments.filter((a) => a.contentType.toLowerCase().startsWith("text/")), [allOtherAttachments]);
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Latest Run Date:{" "}
                {loadingCurrent ? (<span className="text-muted-foreground">Loading...</span>) : currentRun?.run?.timestamp ? (<span className="font-medium">
                    ...
                    Error loading date
                  </span>) : (<span className="font-medium">Not available</span>)}
              </p>
          ...
          <Tabs defaultValue="steps" className="w-full">
            ...
            <TabsContent value="steps" className="mt-4 p-1 md:p-4 border rounded-lg bg-card shadow-inner">
              ...
              {test.errorMessage && (<div className="mb-4 p-3 md:p-0">
                  ...
                  <CodeBlockWithCopy rawContent={test.errorMessage} className="bg-destructive/10 rounded-lg overflow-hidden" preClassName="p-4"/>
                </div>)}
              ...
              <Tabs defaultValue="sub-screenshots" className="w-full">
                <ScrollArea className="w-full whitespace-nowrap rounded-lg">
                  <TabsList className="inline-grid w-max grid-flow-col mb-4 rounded-lg">
                    <TabsTrigger value="sub-screenshots" disabled={screenshotDisplayItems.length === 0}>
                      <ImageIcon className="h-4 w-4 mr-2"/>
                      Screenshots ({screenshotDisplayItems.length})
                    </TabsTrigger>
                    <TabsTrigger value="sub-video" disabled={videoDisplayItems.length === 0}>
                ...
                <TabsContent value="sub-screenshots" className="mt-4">
                  ...
                  {screenshotDisplayItems.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <TabsContent value="sub-video" className="mt-4">
                  ...
                  <div className="space-y-4">
                    {videoDisplayItems.length > 0 ? (videoDisplayItems.map((attachment, index) => (<div key={`video-${index}`} className="p-4 border rounded-lg bg-muted/30 shadow-sm flex items-center justify-between">
                ...
                <TabsContent value="sub-trace" className="mt-4">
                  ...
                  {traceDisplayItem ? (<div className="p-4 border rounded-lg bg-muted/30 space-y-3 shadow-sm">
            ...
            <TabsContent value="logs" className="mt-4 p-4 border rounded-lg bg-card space-y-6 shadow-inner">
              <div>
                ...
                <ScrollArea className="h-48 w-full rounded-lg border p-0 bg-muted/30 shadow-sm">
                  <CodeBlockWithCopy rawContent={test.stdout &&
            Array.isArray(test.stdout) &&
            test.stdout.length > 0
            ? test.stdout.join("\n")
            : "No standard output logs captured for this test."} preClassName="break-words"/>
                </ScrollArea>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-foreground mb-3 flex items-center">
                ...
                <ScrollArea className="h-48 w-full rounded-lg border bg-destructive/5 shadow-sm">
                  <CodeBlockWithCopy rawContent={test.errorMessage || "No errors captured for this test."}/>
                </ScrollArea>
              </div>
            </TabsContent>
