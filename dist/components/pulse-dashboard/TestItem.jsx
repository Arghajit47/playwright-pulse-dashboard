...
export function TestItem({ test }) {
    ...
    const severityAnnotation = useMemo(() => {
        if (!test.annotations || !Array.isArray(test.annotations))
            return null;
        const severity = test.annotations.find((ann) => ann.type === "pulse_severity");
      ...
      {hasDetailsInAccordion && (<Accordion type="single" collapsible className="w-full mt-2">
          <AccordionItem value="details" className="border-none">
            ...
            <AccordionContent className="pt-2 pl-2 pr-2 pb-1 bg-muted/30 rounded-lg">
              ...
              {!test.errorMessage && quickLookScreenshots.length === 0 && (<p className="text-xs text-muted-foreground">
