...
export function FailureCategorizationView() {
    ...
    const categorizedFailures = useMemo(() => {
        if (!currentRun?.results)
            return { categories: [], totalFailed: 0 };
        // Get all failed tests
        const allFailedTests = currentRun.results.filter((test) => test.status === 'failed' || test.status === 'timedOut');
        // Deduplicate by test name - each unique test name should appear once
        const uniqueFailedTestsMap = new Map();
    ...
    return (<div className="space-y-6">
      ...
      {categorizedFailures.categories.map((group, index) => {
            ...
            return (<Card key={group.categoryName} className="shadow-lg hover:shadow-2xl transition-all duration-300 rounded-xl border-0 bg-gradient-to-br from-card via-card to-card/95 group">
            <CardHeader className="border-b border-border/50 bg-muted/20">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {categoryConfig?.description && (<CardDescription className="text-xs mt-1">
              ...
              {group.categoryName === OTHER_ERRORS_CATEGORY &&
                    ...
                    <p className="text-xs text-foreground font-mono">
                      <span dangerouslySetInnerHTML={{ __html: exampleErrorHtml }}/>
                      {showEllipsis ? "..." : ""}
                    </p>
                  </div>)}
            </CardHeader>
                  ...
                  {group.tests.map((test, testIndex) => (<AccordionItem value={`${group.categoryName}-${test.id}-${testIndex}`} key={`${group.categoryName}-${test.id}-${testIndex}`} className="border border-border/50 rounded-lg bg-card hover:bg-muted/30 transition-all duration-200 hover:shadow-md overflow-hidden">
                      <AccordionTrigger className="px-4 py-3 text-left hover:no-underline group/trigger">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm text-foreground group-hover/trigger:text-primary transition-colors truncate" title={formatTestName(test.name)}>
                              {formatTestName(test.name)}
                            </p>
                            <p className="text-xs text-muted-foreground mt-0.5 truncate" title={test.suiteName}>
                      ...
                      <AccordionContent className="px-4 pb-4 pt-2 bg-muted/10">
                        <div className="space-y-3">
                          <Link href={`/test/${test.id}`} className="inline-flex items-center gap-1 text-xs text-primary hover:underline font-medium">
      ...
      {categorizedFailures.categories.length === 0 && (<Alert className="shadow-lg rounded-xl border-0 bg-gradient-to-br from-muted/50 to-card">
