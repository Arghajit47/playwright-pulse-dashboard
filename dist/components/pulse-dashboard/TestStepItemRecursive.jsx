...
import { CheckCircle2, XCircle, AlertCircle, Clock, Info, Code2, Copy, Check, } from "lucide-react";
// --- END: Updated Import ---
...
const CodeBlockWithCopy = ({ rawContent, className, preClassName, }) => {
    ...
    return (<div className={cn("relative", className)}>
      ...
      <Button size="icon" variant="ghost" className="absolute top-1 right-1 h-6 w-6" // Adjusted for smaller size
     onClick={handleCopy}>
    ...
    const stepContent = (<div className={`flex flex-col space-y-2 ${level > 0 ? "pl-4 border-l border-muted ml-2" : ""}`}>
      <div className="flex items-center justify-between space-x-2">
        <div className="flex items-center space-x-2 min-w-0">
          ...
          <span className="text-sm font-medium text-foreground truncate" title={step.title}>
            {step.title}
          </span>
          {step.isHook && (<Badge variant="outline" className="text-xs px-1.5 py-0.5">
              {step.hookType || "hook"}
            </Badge>)}
        </div>
      ...
      {step.errorMessage && (<div className="pl-6 mt-1">
          ...
          {/* --- START: Modified Code --- */}
          <CodeBlockWithCopy rawContent={step.errorMessage} className="bg-destructive/10 rounded-md max-h-32 overflow-y-auto" preClassName="text-xs p-2"/>
          {/* --- END: Modified Code --- */}
        </div>)}
    </div>);
    if (hasSubSteps) {
