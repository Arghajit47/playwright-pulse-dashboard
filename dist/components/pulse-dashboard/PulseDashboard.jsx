        ...
        <div className="container mx-auto p-6 md:p-8 space-y-6 min-h-screen flex flex-col rounded-xl shadow-lg bg-background">
          <header className="mb-0">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Latest Run Date:{" "}
              {loadingCurrent ? (<span className="text-muted-foreground">Loading...</span>) : currentRun?.run?.timestamp ? (<span className="text-muted-foreground font-medium">{new Date(currentRun.run.timestamp).toLocaleString()}</span>) : errorCurrent ? (<span className="text-destructive font-medium">Error loading data</span>) : (<span className="text-muted-foreground font-medium">Not available</span>)}
            </p>
          </header>
