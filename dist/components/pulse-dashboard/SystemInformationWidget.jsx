...
import { Server, Cpu, MemoryStick, Cog, Puzzle, Play, Laptop, Info, Github, Briefcase, Globe, Compass, Chrome, HardDrive } from 'lucide-react';
const getIcon = (key) => {
    ...
    if (lowerKey.includes('provider') && (key.toLowerCase().includes('ci') || key.toLowerCase().includes('cd')))
        return <Github className="h-4 w-4 text-primary"/>;
    if (lowerKey.includes('job') && (key.toLowerCase().includes('ci') || key.toLowerCase().includes('cd')))
...
const renderEnvironmentValue = (value, keyPrefix = '') => {
    ...
    if (Array.isArray(value)) {
        ...
        {value.map((item, index) => (<li key={`${keyPrefix}-arr-${index}`} className="text-sm text-foreground">{renderEnvironmentValue(item, `${keyPrefix}-arr-item-${index}`)}</li>))}
      </ul>);
    }
...
export function SystemInformationWidget({ environmentInfo, loading }) {
    ...
    return (<Card className="shadow-lg rounded-xl mt-6 hover:shadow-xl transition-shadow duration-300">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-primary flex items-center">
          <Server className="h-5 w-5 mr-2"/> System Information
        </CardTitle>
      ...
      <CardContent className="space-y-3 pr-2">
        {Object.entries(environmentInfo).map(([key, value]) => (<div key={key} className="flex items-start space-x-3 p-2 rounded-md hover:bg-muted/30 transition-colors">
