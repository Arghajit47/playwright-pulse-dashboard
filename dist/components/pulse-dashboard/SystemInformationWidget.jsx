...
import { Server, Cpu, MemoryStick, Cog, Puzzle, Play, Laptop, Info, Github, Briefcase, Globe, Compass, Chrome, HardDrive, Layers, } from "lucide-react";
...
const renderEnvironmentValue = (value, keyPrefix = "") => {
    ...
    if (Array.isArray(value)) {
        return (<ul className="list-disc list-inside pl-4">
        {value.map((item, index) => (<li key={`${keyPrefix}-arr-${index}`} className="text-sm text-foreground">
            {renderEnvironmentValue(item, `${keyPrefix}-arr-item-${index}`)}
          </li>))}
      </ul>);
    }
    if (typeof value === "object" && value !== null) {
        return (<div className="ml-4 mt-1 space-y-1 border-l pl-3 border-border">
        {Object.entries(value).map(([key, val]) => (<div key={`${keyPrefix}-${key}`} className="flex items-start space-x-2">
...
const renderEnvironmentGrid = (envInfo, keyPrefix = "") => {
...
export function SystemInformationWidget({ environmentInfo, loading, }) {
    const [activeShardTab, setActiveShardTab] = useState("0");
    if (loading) {
        return (<Card className="shadow-lg rounded-xl mt-6">
        <CardHeader>
          <Skeleton className="h-6 w-48 mb-1"/>
          <Skeleton className="h-4 w-3/4"/>
        </CardHeader>
        <CardContent className="space-y-3">
          {[...Array(4)].map((_, i) => (<div key={i} className="flex items-center space-x-3">
    ...
    const isEmptyEnvironment = !environmentInfo ||
    ...
    return (<Card className="shadow-lg rounded-xl mt-6 hover:shadow-2xl transition-all duration-300 border-0 bg-gradient-to-br from-card via-card to-card/95 group">
        ...
        <div className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-base font-semibold text-foreground group-hover:text-primary transition-colors flex items-center">
              ...
              {isShardedEnvironment && (<span className="ml-2 text-xs font-normal text-muted-foreground flex items-center gap-1">
          ...
          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center">
            {isShardedEnvironment ? (<Layers className="h-4 w-4 text-primary"/>) : (<Laptop className="h-4 w-4 text-primary"/>)}
          </div>
        </div>
      </CardHeader>
      <CardContent className="pt-0">
        {isShardedEnvironment ? (<Tabs value={activeShardTab} onValueChange={setActiveShardTab} className="w-full">
            <TabsList className="w-fit max-w-full h-auto p-1 bg-muted/50 flex justify-start overflow-x-auto">
              {environmentInfo.map((_, index) => (<TabsTrigger key={index} value={String(index)} className="text-xs px-3 py-1.5 data-[state=active]:bg-primary/10 flex-shrink-0">
                  Shard {index + 1}
                </TabsTrigger>))}
            </TabsList>
            {environmentInfo.map((shardEnv, index) => (<TabsContent key={index} value={String(index)} className="mt-4">
                {renderEnvironmentGrid(shardEnv, `shard-${index}-`)}
              </TabsContent>))}
          </Tabs>) : (renderEnvironmentGrid(environmentInfo))}
      </CardContent>
    </Card>);
}
//# sourceMappingURL=SystemInformationWidget.jsx.map