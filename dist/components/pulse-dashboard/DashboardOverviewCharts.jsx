...
import { Terminal, CheckCircle, Info, Chrome, Globe, Compass } from 'lucide-react';
...
export function DashboardOverviewCharts({ currentRun, loading, error }) {
    ...
    const testDistributionChartRef = useRef(null);
    const browserChartRef = useRef(null);
    const failedDurationChartRef = useRef(null);
    const slowestTestsChartRef = useRef(null);
    const testsPerSuiteChartRef = useRef(null);
    const onPieEnter = React.useCallback((_data, index) => {
        setActiveIndex(index);
    }, [setActiveIndex]);
    if (loading) {
        return (<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mt-6">
        {[...Array(4)].map((_, i) => (<Card key={i} className="shadow-md">
            <CardHeader>
