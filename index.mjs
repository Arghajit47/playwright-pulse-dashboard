...
// Helper functions (Server-side only for data processing)
...
function generateHTML(reportData, trendData = null) {
  ...
  // Server-Side helpers for Test History Tab, as it's fully rendered on the server
...
<body>
    ...
    // --- START: Client-side Generation Functions ---
    // These functions are now defined in the client's scope and use window._pulseData
    ...
    function ansiToHtml(text) {
      ...
      const codes = {
        ...
        1: "font-weight:bold", 2: "opacity:0.6", 3: "font-style:italic", 4: "text-decoration:underline",
        30: "color:#000", 31: "color:#d00", 32: "color:#0a0", 33: "color:#aa0", 34: "color:#00d", 35: "color:#a0a", 36: "color:#0aa", 37: "color:#aaa", 39: "color:inherit",
        40: "background-color:#000", 41: "background-color:#d00", 42: "background-color:#0a0", 43: "background-color:#aa0", 44: "background-color:#00d", 45: "background-color:#a0a", 46: "background-color:#0aa", 47: "background-color:#aaa", 49: "background-color:inherit",
        90: "color:#555", 91: "color:#f55", 92: "color:#5f5", 93: "color:#ff5", 94: "color:#55f", 95: "color:#f5f", 96: "color:#5ff", 97: "color:#fff",
      };
    ...
    function convertPlaywrightErrorToHTML(str) {
        if (!str) return "";
        return str.replace(/<red>/g, '<span style="color: red;">').replace(/<green>/g, '<span style="color: green;">').replace(/<dim>/g, '<span style="opacity: 0.6;">').replace(/<intensity>/g, '<span style="font-weight: bold;">').replace(/<\\/color>/g, "</span>").replace(/<\\/intensity>/g, "</span>").replace(/\\n/g, "<br>");
    }
    ...
    function generatePieChartFromData(data, chartWidth = 300, chartHeight = 300) {
      ...
      {
          chart: { type: 'pie', width: \${chartWidth}, height: \${chartHeight - 40}, backgroundColor: 'transparent', plotShadow: false, spacingBottom: 40 },
          title: { text: '\${passedPercentage}%', align: 'center', verticalAlign: 'middle', y: 5,  style: { fontSize: '\${centerTitleFontSize}', fontWeight: 'bold', color: 'var(--primary-color)' } },
          subtitle: { text: 'Passed', align: 'center', verticalAlign: 'middle', y: 25,  style: { fontSize: '\${centerSubtitleFontSize}', color: 'var(--text-color-secondary)' } },
          tooltip: { pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b> ({point.y})', backgroundColor: 'rgba(10,10,10,0.92)', borderColor: 'rgba(10,10,10,0.92)', style: { color: '#f5f5f5' } },
          legend: { layout: 'horizontal', align: 'center', verticalAlign: 'bottom', itemStyle: { color: 'var(--text-color)', fontWeight: 'normal', fontSize: '12px' } },
          plotOptions: { pie: { allowPointSelect: true, cursor: 'pointer', borderWidth: 3, borderColor: 'var(--card-background-color)', states: { hover: {} } } },
    ...
    function generateEnvironmentDashboardFromData(environment, dashboardHeight = 600) {
      ...
      const cardContentPadding = 16;
      return \`
        <div class="environment-dashboard-wrapper" id="\${dashboardId}">
          <div class="env-dashboard-header">
            <div> <h3 class="env-dashboard-title">System Environment</h3> <p class="env-dashboard-subtitle">Snapshot of the execution environment</p> </div>
            <span class="env-chip env-chip-primary">\${environment.host}</span>
          </div>
          <div class="env-card">
            <div class="env-card-header"> <svg viewBox="0 0 24 24"><path d="M4 6h16V4H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8h-2v10H4V6zm18-2h-4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2H6a2 2 0 0 0-2 2v2h20V6a2 2 0 0 0-2-2zM8 12h8v2H8v-2zm0 4h8v2H8v-2z"/></svg> Hardware </div>
            <div class="env-card-content">
              <div class="env-detail-row"> <span class="env-detail-label">CPU Model</span> <span class="env-detail-value">\${environment.cpu.model}</span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">CPU Cores</span> <span class="env-detail-value"> <div class="env-cpu-cores"> \${Array.from({ length: Math.max(0, environment.cpu.cores || 0) }, (_, i) => \`<div class="env-core-indicator \${ i >= (environment.cpu.cores >= 8 ? 8 : environment.cpu.cores) ? "inactive" : "" }" title="Core \${i + 1}"></div>\` ).join("")} <span>\${environment.cpu.cores || "N/A"} cores</span> </div> </span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">Memory</span> <span class="env-detail-value">\${formattedMemory}</span> </div>
            </div>
          </div>
          <div class="env-card">
            <div class="env-card-header"> <svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-0.01 18c-2.76 0-5.26-1.12-7.07-2.93A7.973 7.973 0 0 1 4 12c0-2.21.9-4.21 2.36-5.64A7.994 7.994 0 0 1 11.99 4c4.41 0 8 3.59 8 8 0 2.76-1.12 5.26-2.93 7.07A7.973 7.973 0 0 1 11.99 20zM12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4z"/></svg> Operating System </div>
            <div class="env-card-content">
              <div class="env-detail-row"> <span class="env-detail-label">OS Type</span> <span class="env-detail-value">\${ environment.os.split(" ")[0] === "darwin" ? "darwin (macOS)" : environment.os.split(" ")[0] || "Unknown" }</span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">OS Version</span> <span class="env-detail-value">\${ environment.os.split(" ")[1] || "N/A" }</span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">Hostname</span> <span class="env-detail-value" title="\${environment.host}">\${environment.host}</span> </div>
            </div>
          </div>
          <div class="env-card">
            <div class="env-card-header"> <svg viewBox="0 0 24 24"><path d="M9.4 16.6L4.8 12l4.6-4.6L8 6l-6 6 6 6 1.4-1.4zm5.2 0l4.6-4.6-4.6-4.6L16 6l6 6-6 6-1.4-1.4z"/></svg> Node.js Runtime </div>
            <div class="env-card-content">
              <div class="env-detail-row"> <span class="env-detail-label">Node Version</span> <span class="env-detail-value">\${environment.node}</span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">V8 Engine</span> <span class="env-detail-value">\${environment.v8}</span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">Working Dir</span> <span class="env-detail-value" title="\${environment.cwd}">\${ environment.cwd.length > 25 ? "..." + environment.cwd.slice(-22) : environment.cwd }</span> </div>
            </div>
          </div>
          <div class="env-card">
            <div class="env-card-header"> <svg viewBox="0 0 24 24"><path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM19 18H6c-2.21 0-4-1.79-4-4s1.79-4 4-4h.71C7.37 8.69 9.48 7 12 7c2.76 0 5 2.24 5 5v1h2c1.66 0 3 1.34 3 3s-1.34 3-3 3z"/></svg> System Summary </div>
            <div class="env-card-content">
              <div class="env-detail-row"> <span class="env-detail-label">Platform Arch</span> <span class="env-detail-value"> <span class="env-chip \${ environment.os.includes("darwin") && environment.cpu.model.toLowerCase().includes("apple") ? "env-chip-success" : "env-chip-warning" }"> \${ environment.os.includes("darwin") && environment.cpu.model.toLowerCase().includes("apple") ? "Apple Silicon" : environment.cpu.model.toLowerCase().includes("arm") || environment.cpu.model.toLowerCase().includes("aarch64") ? "ARM-based" : "x86/Other" } </span> </span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">Memory per Core</span> <span class="env-detail-value">\${ environment.cpu.cores > 0 ? (parseFloat(environment.memory) / environment.cpu.cores).toFixed(2) + " GB" : "N/A" }</span> </div>
              <div class="env-detail-row"> <span class="env-detail-label">Run Context</span> <span class="env-detail-value">CI/Local Test</span> </div>
            </div>
          </div>
        </div>
      \`;
    ...
    function generateSuitesWidgetFromData(suitesData) {
      ...
      return \`
        <div class="suites-widget">
          <div class="suites-header"> <h2>Test Suites</h2> <span class="summary-badge">\${suitesData.length} suites • \${suitesData.reduce((sum, suite) => sum + suite.count, 0)} tests</span> </div>
          <div class="suites-grid">
            \${suitesData.map((suite) => \`
              <div class="suite-card status-\${suite.statusOverall}">
                <div class="suite-card-header"> <h3 class="suite-name" title="\${sanitizeHTML(suite.name)} (\${sanitizeHTML(suite.browser)})">\${sanitizeHTML(suite.name)}</h3> </div>
                <div>🖥️ <span class="browser-tag">\${sanitizeHTML(suite.browser)}</span></div>
                <div class="suite-card-body">
                  <span class="test-count">\${suite.count} test\${suite.count !== 1 ? "s" : ""}</span>
                  <div class="suite-stats">
                    \${suite.passed > 0 ? \`<span class="stat-passed" title="Passed"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="bi bi-check-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/></svg> \${suite.passed}</span>\` : ""}
                    \${suite.failed > 0 ? \`<span class="stat-failed" title="Failed"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z"/></svg> \${suite.failed}</span>\` : ""}
                    \${suite.skipped > 0 ? \`<span class="stat-skipped" title="Skipped"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16"><path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/></svg> \${suite.skipped}</span>\` : ""}
                  </div>
                </div>
              </div>
            \`).join("")}
    ...
    function generateTestRunSummaryTabHTML() {
      ...
      let uniqueBrowsers = Array.from(new Set((results || []).map(test => test.browser || "unknown")));
      let browserOptionsHTML = \`<option value="">All Browsers</option>\`.concat(
        uniqueBrowsers.map(browser => \`<option value="\${sanitizeHTML(browser)}">\${sanitizeHTML(browser)}</option>\`).join("")
      );
            ...
            return \`
              <div class="compact-failure-item">
                ...
                <div class="full-error-details" style="display: none;">
                  <div class="full-error-content">\${formatPlaywrightError(test.errorMessage || "No detailed error message available")}</div>
                </div>
              </div>
            \`;
          }).join('')}
    ...
    function generateTestDetailsHTML(testIndex) {
      ...
      const generateStepsHTML = (steps, depth = 0) => {
        ...
        return steps.map((step) => {
          ...
          return \`
          <details class="step-item" style="--depth: \${depth};">
              <summary class="step-header \${stepClass}">
              ...
              <div class="step-details">
                ...
                \${step.errorMessage ? \`<div class="test-error-summary"> \${step.stackTrace ? \`<div class="stack-trace">\${formatPlaywrightError(step.stackTrace)}</div>\` : ""} <button class="copy-error-btn" onclick="copyErrorToClipboard(this)">Copy Error Prompt</button> </div>\` : ""}
      ...
      if (test.screenshots && test.screenshots.length > 0) {
        html += \`<div class="attachments-section"><h4>Screenshots</h4><div class="attachments-grid">\${test.screenshots.map((screenshot, index) => \` <div class="attachment-item"> <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="\${screenshot}" alt="Screenshot \${index + 1}" class="lazy-load-image"> <div class="attachment-info"> <div class="trace-actions"> <a href="\${screenshot}" target="_blank" download="screenshot-\${index}.png">Download</a> </div> </div> </div> \`).join("")}</div></div>\`;
      }
      
      if (test.videoPath && test.videoPath.length > 0) {
        html += \`<div class="attachments-section"><h4>Videos</h4><div class="attachments-grid">\${test.videoPath.map((video, index) => \` <div class="attachment-item video-item"> <video controls preload="none" class="lazy-load-video"> <source data-src="\${video.dataUri}" type="\${video.mimeType}"> </video> <div class="attachment-info"> <div class="trace-actions"> <a href="\${video.dataUri}" target="_blank" download="video-\${index}.\${video.extension}">Download</a> </div> </div> </div> \`).join("")}</div></div>\`;
      }
      ...
      if (test.codeSnippet) {
        html += \`<div class="code-section"><h4>Code Snippet</h4><pre><code>\${sanitizeHTML(test.codeSnippet)}</code></pre></div>\`;
      }
      
      return html;
    }

    // --- END: Client-side Generation Functions ---
    ...
    function loadTabContent(tabId) {
        ...
        // Re-run lazy loader for any new charts/media
        if ('IntersectionObserver' in window) {
            const newLazyElements = contentEl.querySelectorAll('.lazy-load-chart, .lazy-load-image, .lazy-load-video, .lazy-load-attachment');
            const lazyObserver = getLazyObserver();
    ...
    function initializeReportInteractivity() {
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                // Lazy load content on first click
                ...
                // Re-initialize filters for newly loaded tab
                if (tabId === 'test-runs') {
                    initializeTestRunFilters();
                }
            });
        });

        document.querySelector('.tab-button[data-tab="dashboard"]').click(); // Activate dashboard by default
    }

    // This function will set up event listeners for the Test Run Summary filters
    function initializeTestRunFilters() {
        ...
        // Setup expand/collapse for test cases
        document.querySelector('#test-runs').addEventListener('click', function(event) {
          ...
          if (header) {
            ...
            if (!isExpanded) {
              ...
              if (content.innerHTML === '') { // Lazy load details
                content.innerHTML = generateTestDetailsHTML(testIndex);
                // Trigger lazy loading for media inside the newly added details
                 if ('IntersectionObserver' in window) {
                    const newLazyElements = content.querySelectorAll('.lazy-load-image, .lazy-load-video, .lazy-load-attachment');
                    const lazyObserver = getLazyObserver();
                    newLazyElements.forEach(el => lazyObserver.observe(el));
                }
              }
        ...
        // Expand/Collapse All
        document.getElementById('expand-all-tests')?.addEventListener('click', () => setAllTestRunDetailsVisibility(true));
        document.getElementById('collapse-all-tests')?.addEventListener('click', () => setAllTestRunDetailsVisibility(false));
    }
    
    function setAllTestRunDetailsVisibility(expand) {
      document.querySelectorAll('#test-runs .test-case-header').forEach(header => {
        const content = header.nextElementSibling;
        const isExpanded = header.getAttribute('aria-expanded') === 'true';

        if ((expand && !isExpanded) || (!expand && isExpanded)) {
            header.click(); // This will trigger the lazy-loading logic if necessary
        }
      });
    }

    let lazyObserverInstance;
    function getLazyObserver() {
                ...
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        if (element.classList.contains('lazy-load-image')) {
                            if (element.dataset.src) { element.src = element.dataset.src; element.removeAttribute('data-src'); }
                        } else if (element.classList.contains('lazy-load-video')) {
                            const source = element.querySelector('source');
                            if (source && source.dataset.src) { source.src = source.dataset.src; source.removeAttribute('data-src'); element.load(); }
                        } else if (element.classList.contains('lazy-load-attachment')) {
                            if (element.dataset.href) { element.href = element.dataset.href; element.removeAttribute('data-href'); }
        ...
        return lazyObserverInstance;
    }
    
    document.addEventListener('DOMContentLoaded', () => {
        initializeReportInteractivity();

        // Initialize Test History Filters (since it's not lazy loaded)
        ...
        // Initial trigger for lazy observer on server-rendered content (Test History charts)
        const lazyObserver = getLazyObserver();
        document.querySelectorAll('.lazy-load-chart').forEach(el => lazyObserver.observe(el));
    });
    ...
    function copyLogContent(elementId, button) {
        ...
        }).catch(err => { button.textContent = 'Failed'; setTimeout(() => { button.textContent = 'Copy'; }, 2000); });
    }
    ...
    function getAIFix(button) {
        ...
        try {
            ...
            .then(response => {
                if (!response.ok) return response.text().then(text => { throw new Error(\`API Error (\${response.status}): \${text || response.statusText}\`); });
                return response.text();
            })
            .then(text => {
                if (!text) throw new Error("Empty response from AI analyzer.");
                try { return JSON.parse(text); } catch (e) { throw new Error(\`Invalid JSON from AI: \${e.message}\`); }
            })
            .then(data => {
                ...
                if (data.suggestedFixes && data.suggestedFixes.length > 0) {
                    ...
                    data.suggestedFixes.forEach(fix => {
                        suggestionsHtml += \`<div class="suggestion-item" style="margin-bottom: 22px; border-left: 3px solid var(--accent-color-alt); padding-left: 15px;"><p style="margin: 0 0 8px 0; font-weight: 500;">\${escapeHtml(fix.description)}</p>\${fix.codeSnippet ? \`<div class="code-section"><pre><code>\${escapeHtml(fix.codeSnippet)}</code></pre></div>\` : ''}</div>\`;
                    });
                    suggestionsHtml += '</div>';
                } else {
